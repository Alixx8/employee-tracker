...
async function getRatings(dbConn, empId) {
    return await dbConn.query('SELECT * FROM ratings WHERE "employeeID" = $1', [empId]);
}
...
async function updateRating(dbConn, empId, id, rating) {
    return await dbConn.query('UPDATE ratings SET days = $1, rating = $2 WHERE id = $3 AND "employeeID" = $4 RETURNING *', 
...
async function deleteRating(dbConn, empId, id) {
    return await dbConn.query('DELETE FROM ratings WHERE id = $1 AND "employeeID" = $2', 
        [id, empId]
    );
}
